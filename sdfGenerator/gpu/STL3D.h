#ifndef SDFGENERATOR_GPU_STL3D_H
#define SDFGENERATOR_GPU_STL3D_H
...
public:
	template <typename T, typename Coord>
	void calcSDF(
			T* sdf, const int num_cell[3], const Coord* const coord[3],
			const int sign_solid = 1 ) const;
};
...
void STL3D::calcSDF(
		T* sdf, const int num_cell[3], const Coord* const coord[3],
		const int sign_solid ) const
{
	const int num_cell_total = num_cell[0] * num_cell[1] * num_cell[2];
...
#endif // SDFGENERATOR_GPU_STL3D_H
